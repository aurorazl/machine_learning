...
def create_dataset(path_inp,path_targ):
    with open(path_inp,'r') as f:
        inp = f.read().strip().split('\n')
    with open(path_targ, 'r') as f:
        targ = f.read().strip().split('\n')
    word_pairs = [[preprocess_sentence(inp[i]),preprocess_sentence(targ[i])]  for i in range(len(inp))]
...
def tokenize(inp_lang,targ_lang):
  lang_tokenizer_inp = tf.keras.preprocessing.text.Tokenizer(filters='',num_words=number_word)     # 保留前出现次数最多的
  lang_tokenizer_targ = tf.keras.preprocessing.text.Tokenizer(filters='',num_words=number_word)
  lang_tokenizer_inp.fit_on_texts(inp_lang)
  lang_tokenizer_targ.fit_on_texts(targ_lang)
  return lang_tokenizer_inp,lang_tokenizer_targ

def load_dataset(path_train_inp,path_train_targ):
    # creating cleaned input, output pairs
    inp_lang, targ_lang = create_dataset(path_train_inp,path_train_targ)

    inp_lang_tokenizer,targ_lang_tokenizer = tokenize(inp_lang,targ_lang)

    return inp_lang_tokenizer, targ_lang_tokenizer

inp_lang_tokenizer, targ_lang_tokenizer = load_dataset(path_train_inp, path_train_targ)
    ...
    with open(path_train_inp,'r') as f_inp:
        with open(path_train_targ, 'r') as f_targ:
            for inp,targ in zip(f_inp,f_targ):   # while inp and targ f.readline()有可能遇到一行为空的情况
                inp_tensor = inp_lang_tokenizer.texts_to_sequences([preprocess_sentence(inp.strip())])
                targ_tensor = targ_lang_tokenizer.texts_to_sequences([preprocess_sentence(targ.strip())])
                if len(inp_tensor)<buckets[bucket_id][0] and len(targ_tensor)<buckets[bucket_id][1]:
                    X_batch.append(inp_tensor[0])
                    y_batch.append(targ_tensor[0])
                    cnt += 1
                if cnt == batch_size:
                    yield tf.keras.preprocessing.sequence.pad_sequences(X_batch,padding='post',maxlen=buckets[bucket_id][0]),tf.keras.preprocessing.sequence.pad_sequences(y_batch,padding='post',maxlen=buckets[bucket_id][1])
                    cnt = 0
                    X_batch = []
                    y_batch = []
