...
dataset = dataset.batch(BATCH_SIZE, drop_remainder=True)        # 最后不够64个时丢弃
...
checkpoint_dir = './training_checkpoints/spanishToEnglish'
...
#     total_loss = 0
#
#     for (batch, (inp, targ)) in enumerate(dataset.take(steps_per_epoch)):        # take会无限循环
...
#     # while True:
#     #     try:
#     #         (inp, targ) = sess.run(data_iterator.get_next())
#     #         batch_loss = train_step(inp, targ, enc_hidden)
#     #         total_loss += batch_loss
#     #         batch +=1
#     #         if batch % 100 == 0:
...
def evaluate(sentence):
    ...
    for t in range(max_length_targ):        # 遍历最大长度
...
translate(u'Acabo de llegar aquí.')
